...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        # ones from the module
        global_orient = (global_orient if global_orient is not None else
                         self.global_orient.repeat(batch_size, 1))
        body_pose = body_pose if body_pose is not None else self.body_pose
        betas = betas if betas is not None else self.betas.repeat(batch_size, 1)

        left_hand_pose = (left_hand_pose if left_hand_pose is not None else
                          self.left_hand_pose.repeat(batch_size, 1))
        right_hand_pose = (right_hand_pose if right_hand_pose is not None else
                           self.right_hand_pose.repeat(batch_size, 1))
