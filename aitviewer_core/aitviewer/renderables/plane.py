...
from aitviewer_core.aitviewer.shaders import get_chessboard_program, get_smooth_lit_with_edges_program
from aitviewer_core.aitviewer.utils import set_lights_in_program, set_material_properties
...
class ChessboardPlane(Node):
    ...
    def update_floor(self, rot):
        self.vertices = np.dot(rot, self.vertices.T).T
        self.normals = np.dot(rot, self.normals.T).T
